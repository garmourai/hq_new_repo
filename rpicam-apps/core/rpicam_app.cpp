...
{
	auto requests_start_time = std::chrono::high_resolution_clock::now();

	// This makes all the Request objects that we shall need.
	makeRequests();

	auto requests_stop_time = std::chrono::high_resolution_clock::now();
	auto requests_exec_duration = std::chrono::duration_cast<std::chrono::milliseconds>(requests_stop_time - requests_start_time).count();
    std::cout << "Time taken for making requests objects: " << requests_exec_duration << " ms" << std::endl;

	auto controls_start_time = std::chrono::high_resolution_clock::now();
	...
	auto controls_stop_time = std::chrono::high_resolution_clock::now();
	auto controls_exec_duration = std::chrono::duration_cast<std::chrono::milliseconds>(controls_stop_time - controls_start_time).count();    
    std::cout << "Time taken for initializing controls: " << controls_exec_duration << " ms" << std::endl;

	post_processor_.Start();

	auto queue_start_time = std::chrono::high_resolution_clock::now();	

	camera_->requestCompleted.connect(this, &RPiCamApp::requestComplete);

	std::cout << "Number of previous requests: " << requests_.size();
	...
	auto queue_stop_time = std::chrono::high_resolution_clock::now();
	auto queue_exec_duration = std::chrono::duration_cast<std::chrono::milliseconds>(queue_stop_time - queue_start_time).count();
    std::cout << "Time taken for queueing requests: " << queue_exec_duration << " ms" << std::endl;
...
int RPiCamApp::GetMessageQueueSize()
{
	int queue_size = msg_queue_.GetSize();
	return queue_size;
}

int RPiCamApp::GetCompletedRequestsSize()
{
...
std::optional<RPiCamApp::Msg> RPiCamApp::TimedWait(std::chrono::seconds timeout)
{
	return msg_queue_.TimedWait(timeout);
}
