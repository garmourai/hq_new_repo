...
private:
	void encodeBufferDone(void *mem)
	{
		...
		assert(mem == nullptr);
		{
			...
			// CRITICAL: Always call metadata callback if set, regardless of metadata file setting
			// This allows outputs to access SensorTimestamp even when not writing metadata files
