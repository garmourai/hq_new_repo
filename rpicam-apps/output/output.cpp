...
void Output::OutputReady(void *mem, size_t size, int64_t timestamp_us, bool keyframe)
{
	...
	// Get metadata if available (before popping from queue)
	// Note: We check the queue even if not writing metadata file, so outputs can use SensorTimestamp
	libcamera::ControlList *metadata_ptr = nullptr;
	libcamera::ControlList metadata_copy;
	if (!metadata_queue_.empty()) {
		metadata_copy = metadata_queue_.front();
		metadata_ptr = &metadata_copy;
	}

	// Call outputBufferWithMetadata if available, otherwise outputBuffer
	outputBufferWithMetadata(mem, size, last_timestamp_, flags, metadata_ptr);
	...
	// Handle metadata: write to file if requested, and pop from queue to prevent growth
	if (!metadata_queue_.empty())
	{
		...
		metadata_queue_.pop(); // Always pop to prevent queue growth
	}
}
...
void Output::outputBufferWithMetadata(void *mem, size_t size, int64_t timestamp_us, 
                                      uint32_t flags, libcamera::ControlList *metadata)
{
	// Default implementation just calls outputBuffer (for backward compatibility)
...
{
	// Always push metadata to queue (even if not writing metadata file) so outputs can use SensorTimestamp
