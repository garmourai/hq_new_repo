...
###################################
def execute_cycle(source_ip, sink_1_ip, user,
                  source_remote_dir, sink_remote_dir,
                  source_local_dir, sink_1_local_dir,
    ...
    for i, (start_delay, stop_delay) in enumerate(schedule, start=1):
        ...
        is_IDLE = True
        while source_controller.get_state() != "IDLE" or sink_1_controller.get_state() != "IDLE" :
            time.sleep(1)
        ...
        if is_IDLE:   
            ...
            restart_flag = 0
            restart_sink_1 = False
            if new_id != track_index:
                source_controller.change_track_index(new_id)
                status_line_mismatch = f"ID Mismatch!!! source_id:{track_index} sink_1_id:{sink_1_id} "
                mismatch_flag = 1
                ...
                status_line_mismatch = f"ID Matched source_id:{track_index} sink_1_id:{sink_1_id}"
                mismatch_flag = 0
            ...
            print(f"Number of frames captured by source: {source_frame_count}, sink 1: {sink_1_frame_count} ")

            status_line_restart = f" Frame Count Matched source_frame_count:{source_frame_count} sink_1_frame_count:{sink_1_frame_count} "
            if content_1_dict['num_frames'] < threshold:
                status_line_restart = f" Frame Count Mismatch for Sink 1!!! source_frame_count:{source_frame_count} sink_1_frame_count:{sink_1_frame_count} "
                restart_flag = 1

            # if i%2 == 0:
            #     restart_sink_1 = True 
            #     print("Right now restarting before every even capture")
            # restart_sink_1 = False
            if (restart_sink_1):
                if sink_1_controller.restart():
                    print("Sink 1 restarted successfully")
                    time.sleep(1)
                else:
        ...
        time2 = time.time()
        source_controller.get_and_store_offset("sink_1", sink_1_ip)
        ...
        sink_1_stop_ok = False

        num_tries = 0
        while not source_stop_ok and num_tries < MAX_RETRIES:
            source_stop_ok = source_controller.stop_capture(user)
            num_tries += 1
            if not source_stop_ok:
                print(f"[Cycle {i}] Failed to stop source camera. Retrying in 1 second...")
                time.sleep(1)        

        time.sleep(1)

        num_tries = 0
        while not sink_1_stop_ok and num_tries < MAX_RETRIES:
            sink_1_stop_ok = sink_1_controller.stop_capture(user)
            num_tries += 1
            if not sink_1_stop_ok:
                print(f"[Cycle {i}] Failed to stop sink 1 camera. Retrying in 1 second...")
                time.sleep(1)
        ...
        if not (source_stop_ok) or not (sink_1_stop_ok):
...
def main():
    ...
    SCHEDULE = [(random.randint(600, 900), 0) for _ in range(30)]
