...
def slave():
    ...
    try:
        ...
        for i in range(60):
            ...
            # one_shot_video()
            pi_logger.info("waiting for 3 sec")
            time.sleep(3)
            # source_video()
            
            video_capture(filename_path)
...
@app.command()
def video_capture(output):
    # os.system(f"rpicam-vid --frames 3000 -o {output} --save-pts {timestamp_path}") 
